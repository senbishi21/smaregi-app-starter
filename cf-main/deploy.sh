...
# 関数設定
CLOUD_FUNC_NAME="${DEPLOY_ENV}-main"
CLOUD_FUNC_ENTRYPOINT="entrypoint"
...
if [ $DEPLOY_ENV = "qa" ]; then
	gcloud functions deploy $CLOUD_FUNC_NAME \
		...
		--set-env-vars env=${DEPLOY_ENV},projectID=${PROJECT_ID},app_client_id=${QA_APP_CLIENT_ID},app_client_secret=${QA_APP_CLIENT_SECRET} \
	...
	gcloud functions deploy $CLOUD_FUNC_NAME \
		--source $SCRIPT_DIR/deploy\
		--set-env-vars env=${DEPLOY_ENV},projectID=${PROJECT_ID},app_client_id=${PROD_APP_CLIENT_ID},app_client_secret=${PROD_APP_CLIENT_SECRET} \
		--entry-point $CLOUD_FUNC_ENTRYPOINT\
		--memory 128mb \
		--runtime python38 --region asia-northeast1 --trigger-http --allow-unauthenticated &&\
    log_and_notify
